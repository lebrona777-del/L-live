...
/* =========================================================
   VERIFIED OFFICIAL LOGO URLS ALREADY FOUND
   ========================================================= */
...
function scoreCandidate(imageUrl, alt, title, context, team) {
  if (!imageUrl || isBadLogo(imageUrl)) {
    return -9999;
  }

  const teamName = normalizeText(team);
  const urlText = normalizeText(imageUrl);
  ...
  if (altText === teamName) score += 100;
  if (titleText === teamName) score += 90;

  if (altText.includes(teamName)) score += 70;
  if (titleText.includes(teamName)) score += 60;

  if (contextText.includes(teamName)) score += 50;
  if (urlText.includes(teamName)) score += 40;

  if (/\.(png|jpg|jpeg|webp)(\?|$)/i.test(imageUrl)) {
    score += 10;
  }
...
function extractLogos(html, sourceUrl, teams) {
  const result = {};
  ...
  for (const team of teams) {
    ...
    const teamNormalized = normalizeText(team);

    for (const tag of imgTags) {
      const urls = getImageUrlsFromTag(tag);
      ...
      const context = html.substring(
        Math.max(0, tagIndex - 700),
        Math.min(html.length, tagIndex + 700)
      );
      ...
      for (const rawUrl of urls) {
        ...
        if (isBadLogo(absoluteUrl)) continue;
    ...
    /*
      Only accept a logo when the official page gives us
      enough evidence that the image belongs to this team.
    */
    if (bestUrl && bestScore >= 50) {
      result[teamNormalized] = bestUrl;
    }
  }
...
async function getOfficialLogos(teams) {
  const now = Date.now();

  /*
    Cache official logo lookup for 30 minutes.
  */
  if (
    logoCache &&
    now - logoCacheTime < 30 * 60 * 1000
  ) {
    return logoCache;
  }
  ...
  /*
    First: use verified official URLs.
  */
  for (const team of teams) {
    if (OFFICIAL_LOGO_OVERRIDES[team]) {
      logos[normalizeText(team)] =
        OFFICIAL_LOGO_OVERRIDES[team];
    }
  }

  /*
    Second: search official GAFA/GFF pages for anything
    that is not already known.
  */
  const remainingTeams = teams.filter(
    team => !logos[normalizeText(team)]
  );

  if (remainingTeams.length) {
    for (const sourceUrl of OFFICIAL_SOURCES) {
      if (!remainingTeams.length) break;

      const html = await fetchPage(sourceUrl);

      if (!html) continue;

      const extracted = extractLogos(
        html,
        sourceUrl,
        remainingTeams
      );

      for (const [teamName, logoUrl] of Object.entries(
        extracted
      )) {
        if (!logos[teamName]) {
      ...
      /*
        Keep only teams that still need a logo.
      */
      for (let i = remainingTeams.length - 1; i >= 0; i--) {
        if (
          logos[normalizeText(remainingTeams[i])]
        ) {
...
async function addLogos(matches) {
  ...
  return matches.map(match => {
    ...
    const homeLogo =
      logos[normalizeText(match.homeTeam)] || null;

    const awayLogo =
      logos[normalizeText(match.awayTeam)] || null;
...
app.get("/api/matches", async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      source: "L-LIVE",
      logoSource: "GFF / GAFA official pages",
...
app.get("/", (req, res) => {
  res.send(`
    <html>
      <head>
        <meta charset="UTF-8">
        <title>L-LIVE API</title>
      </head>
      <body style="font-family:Arial;padding:30px">
        <h1>⚽ L-LIVE</h1>
        <p>API is running.</p>
        <p><a href="/api/matches">Open matches API</a></p>
