...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    if (!file.mimetype || !file.mimetype.startsWith("image/")) {
      return cb(
        new Error("მხოლოდ სურათის ფაილის ატვირთვა არის დაშვებული.")
      );
    }
...
function requireConfig(res) {
  if (!SUPABASE_URL || !SUPABASE_SECRET_KEY) {
    res.status(500).json({
      ...
      error:
        "Supabase არ არის დაკავშირებული. Vercel-ში დაამატე SUPABASE_URL და SUPABASE_SECRET_KEY."
    });
...
function isAdmin(req) {
  ...
  return (
    cookieValue(req, "llive_admin") === authToken()
  );
}
...
=========================================================
IMPORTANT:
New Supabase Secret Keys are sent through:
apikey: sb_secret_...
=========================================================
*/
...
async function supabaseFetch(endpoint, options = {}) {
  if (!SUPABASE_URL) {
    throw new Error("SUPABASE_URL არ არის მითითებული.");
  }
...
/*
=========================================================
GET STORAGE OBJECT PATH FROM PUBLIC URL
=========================================================
*/
...
async function deleteImage(image) {
  const objectPath = imagePathFromUrl(image);
...
app.post("/api/admin/login", (req, res) => {
  ...
  res.setHeader(
    ...
    `llive_admin=${encodeURIComponent(authToken())}; ` +
      `Path=/; ` +
      `HttpOnly; ` +
      `Secure; ` +
      `SameSite=Lax; ` +
